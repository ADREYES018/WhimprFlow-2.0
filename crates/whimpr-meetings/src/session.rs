...
/// Record a gap in the transcript caused by the machine sleeping.
///
/// Rather than leaving silence or a jump in timestamps with no explanation,
/// this inserts an explicit notice into `transcript.md`.
pub fn record_sleep_gap(dir: &Path, asleep_ms: u64) -> Result<(), String> {
    let path = dir.join("transcript.md");
    let existing = std::fs::read_to_string(&path).unwrap_or_default();
    if existing.trim().is_empty() {
        return Ok(());
    }
    let secs = (asleep_ms + 500) / 1000;
    let mins = secs / 60;
    ...
    let note = format!("_[Machine slept for {duration_str} — audio suspended]_\n\n");
    let mut md = existing.trim_end().to_string();
    md.push_str("\n\n");
    md.push_str(&note);
    crate::store::write(&path, &md)?;
    Ok(())
...
mod tests {
    ...
    #[test]
    fn sleep_gap_is_recorded_in_the_transcript() {
        with_scratch(|dir| {
            let t = transcript(&[(0, "hello before sleep")]);
            write_transcript_md(dir, "Meeting", &t, 0).unwrap();
            record_sleep_gap(dir, 125_000).unwrap();
            let md = std::fs::read_to_string(dir.join("transcript.md")).unwrap();
            assert!(md.contains("hello before sleep"), "{md}");
            assert!(md.contains("_[Machine slept for 2m 5s — audio suspended]_"), "{md}");
        })
    }
}
