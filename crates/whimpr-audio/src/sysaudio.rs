...
pub enum SysAudioError {
    #[error("Screen Recording permission denied")]
    PermissionDenied,
    #[error("System audio is already recording")]
    AlreadyRecording,
    #[error("System audio is not recording")]
    NotRecording,
    #[error("Failed to capture system audio: {0}")]
    Other(String),
}
...
static ACTIVE: Mutex<Option<SysAudioRecorder>> = Mutex::new(None);
...
pub fn start_sysaudio_recording_with_tap(
    ...
    let mut slot = ACTIVE
        .lock()
        .map_err(|_| SysAudioError::Other("sysaudio lock poisoned".into()))?;
    if slot.is_some() {
    ...
    let recorder = SysAudioRecorder::start_with_tap(path, tap).map_err(|e| {
        ...
        {
            SysAudioError::PermissionDenied
        } else {
            SysAudioError::Other(e)
        }
    })?;
    *slot = Some(recorder);
    Ok(())
...
pub fn stop_sysaudio_recording() -> Result<PathBuf, SysAudioError> {
    let recorder = {
        let mut slot = ACTIVE
            .lock()
            .map_err(|_| SysAudioError::Other("sysaudio lock poisoned".into()))?;
        slot.take().ok_or(SysAudioError::NotRecording)?
    };
    let path = recorder.path().clone();
    recorder.stop().map_err(SysAudioError::Other)?;
    Ok(path)
}

/// Whether system-audio is currently capturing.
pub fn is_sysaudio_recording() -> bool {
    ACTIVE
        .lock()
...
mod tests {
    ...
    #[test]
    fn start_sysaudio_recording_with_permission_denied_returns_err() {
        if !has_screen_capture_permission() {
            let res = start_sysaudio_recording(PathBuf::from("/tmp/test_sysaudio.wav"));
            assert_eq!(res, Err(SysAudioError::PermissionDenied));
            assert!(!is_sysaudio_recording());
        }
    }
}
