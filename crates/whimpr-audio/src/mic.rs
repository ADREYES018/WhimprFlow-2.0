...
impl MicRecorder {
    ...
    pub fn start_with_tap(path: PathBuf, tap: Option<Lane>) -> Result<Self, String> {
        ...
        let handle = std::thread::Builder::new()
            ...
            .spawn(move || run_capture(thread_path, thread_stop, thread_muted, ready_tx, tap))
    ...
    /// Mute or unmute live microphone recording.
    pub fn set_muted(&self, muted: bool) {
        self.muted.store(muted, Ordering::SeqCst);
    }

    /// Check if live microphone recording is muted.
    ...
    let stream_res = match sample_format {
        SampleFormat::F32 => {
            ...
            device.build_input_stream(
                &config,
                move |data: &[f32], _| {
                    let is_muted = mu.load(Ordering::Relaxed);
                    let rate = m.observe(frames_in(data.len()));
                    if is_muted {
                        write_samples(&w, (0..data.len()).map(|_| 0i16));
                        if let Some(t) = &t {
                            let zeros = vec![0.0f32; data.len()];
                            t.push(&zeros, channels, rate);
                        }
                    } else {
...
pub enum MicError {
    ...
    PermissionDenied,
    #[error("Microphone is already recording")]
    AlreadyRecording,
    #[error("Microphone is not recording")]
    NotRecording,
    #[error("Failed to capture microphone: {0}")]
    Other(String),
}
...
pub fn stop_mic_recording() -> Result<PathBuf, MicError> {
    let recorder = {
        let mut slot = ACTIVE
            .lock()
            .map_err(|_| MicError::Other("mic lock poisoned".into()))?;
        slot.take().ok_or(MicError::NotRecording)?
    };
    let path = recorder.path().clone();
    recorder.stop().map_err(MicError::Other)?;
    Ok(path)
}
