...
use whimpr_meetings::library::{self, Meeting};
...
pub fn answer(question: &str, on_token: &mut dyn FnMut(&str)) -> Result<LibraryAnswer, String> {
    ...
    let answer = crate::chat::answer_from_library(Path::new(""), &context, question, on_token)?;
...
mod tests {
    use super::*;
    use whimpr_meetings::session::recordings_root;
    use whimpr_meetings::settings::HOME_ENV_LOCK as HOME_LOCK;
        ...
        with_temp_home(|| {
            seed(
                ...
                &[(
                    ...
                    &transcript(
                        "Pricing review",
                        "we went round on the pricing tiers and decided to hold at twenty nine dollars",
                    ),
                )],
            );
            seed(
                ...
                &[(
                    ...
                    &transcript(
                        "Standup",
                        "so what did we do about the deploys, we did them, and what about the flaky test",
                    ),
                )],
            );
            seed(
                ...
                &[(
                    ...
                    &transcript(
                        "Roadmap",
                        "we decided to push the mobile client to Q4",
                    ),
                )],
            );
