...
use whimpr_meetings::library::{meeting_dir, read_meta, source_text, update_meta};
use whimpr_meetings::store;
...
impl StudySettings {
    ...
    pub fn sanitized(&self) -> StudySettings {
        StudySettings {
...
fn store_generated(
    ...
    store::write(&dir.join(file), contents)?;
...
fn cached_settings(dir: &Path, key: &str) -> Option<StudySettings> {
    serde_json::from_value(read_meta(dir).get(key)?.clone()).ok()
}
...
pub fn generate_study_plan(
    ...
    let source = source_text(id)?;
    let plan = crate::chat::generate_study_plan(Path::new(""), &source, &settings)?;
...
pub fn generate_flashcards(
    ...
    if !force {
        if let Some(cached) =
            cached_under(&dir, FLASHCARDS_FILE, FLASHCARDS_SETTINGS_KEY, &settings)
        {
    ...
    let source = source_text(id)?;
    let cards = crate::chat::generate_flashcards(Path::new(""), &source, &settings)?;
...
pub fn generate_quiz(
    ...
    let source = source_text(id)?;
    let questions = crate::chat::generate_quiz(Path::new(""), &source, &settings)?;
...
mod tests {
    ...
    fn with_store<T>(f: impl FnOnce() -> T) -> T {
        let _guard = whimpr_meetings::settings::HOME_ENV_LOCK
            .lock()
    ...
    fn storing_study_settings_leaves_the_rest_of_meta_alone() {
        with_store(|| {
            ...
            let meta = read_meta(dir.as_path());
